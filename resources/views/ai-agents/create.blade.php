...
<head>
    ...
    <title>Add AI Agent - itsLaravel13Setup</title>
    <style>
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            <h1>➕ Add New AI Agent</h1>

            <form action="{{ route('ai-agents.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select id="provider" name="provider" required>
                        ...
                        <option value="openai" {{ old('provider') == 'openai' ? 'selected' : '' }}>OpenAI (GPT)</option>
                        <option value="anthropic" {{ old('provider') == 'anthropic' ? 'selected' : '' }}>Anthropic (Claude)</option>
                        <option value="azure" {{ old('provider') == 'azure' ? 'selected' : '' }}>Azure OpenAI</option>
                        <option value="bedrock" {{ old('provider') == 'bedrock' ? 'selected' : '' }}>AWS Bedrock</option>
                        <option value="groq" {{ old('provider') == 'groq' ? 'selected' : '' }}>GroqCloud</option>
                        <option value="xai" {{ old('provider') == 'xai' ? 'selected' : '' }}>xAI (Grok)</option>
                        <option value="deepseek" {{ old('provider') == 'deepseek' ? 'selected' : '' }}>DeepSeek</option>
                        <option value="mistral" {{ old('provider') == 'mistral' ? 'selected' : '' }}>Mistral AI</option>
                        <option value="ollama" {{ old('provider') == 'ollama' ? 'selected' : '' }}>Ollama (Local)</option>
                        <option value="openrouter" {{ old('provider') == 'openrouter' ? 'selected' : '' }}>OpenRouter</option>
                    </select>
                    @error('provider')
                        <p class="error">{{ $message }}</p>
                    @enderror
                ...
                <div class="form-group">
                    <label for="api_key">API Key <span style="color:#e74c3c;">*</span></label>
                ...
                <div class="form-group">
                    ...
                    <p class="hint">Leave empty to use provider's default model</p>
                    @error('model')
                        <p class="error">{{ $message }}</p>
                    @enderror
                </div>
